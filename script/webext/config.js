...
export function applyConfigToArgv({ argv, argvFromCLI, configFileName, configObject, options }) {
  let newArgv = {
    ...argv,
  }
  ...
  try {
    ...
    if (resolvedFilePath.endsWith('package.json')) {
      configModule = parseJSON(
        await fs.readFile(resolvedFilePath, {
          encoding: 'utf-8',
        }),
      )
    } else {
    ...
    } else if ([ERR_EXPORT_FROM_CJS, ERR_IMPORT_FROM_CJS].includes(error.message)) {
