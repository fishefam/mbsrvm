...
export async function createExtensionRunner(config) {
  switch (config.target) {
    case 'firefox-desktop': {
      const { FirefoxDesktopExtensionRunner } = await import('./firefox-desktop.js')
      return new FirefoxDesktopExtensionRunner(config.params)
    }
    case 'firefox-android': {
      const { FirefoxAndroidExtensionRunner } = await import('./firefox-android.js')
...
export class MultiExtensionRunner {
  ...
  handleReloadResults(results) {
    for (const { reloadError, runnerName, sourceDir } of results) {
...
export function defaultReloadStrategy(
  { artifactsDir, extensionRunner, ignoreFiles, noInput = false, sourceDir, watchFile, watchIgnored },
  { createWatcher = defaultWatcherCreator, kill = process.kill, stdin = process.stdin } = {},
) {
