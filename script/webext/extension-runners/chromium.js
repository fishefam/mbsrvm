...
export class ChromiumExtensionRunner {
  ...
  static async getProfilePaths(chromiumProfile) {
    ...
    if (isProfileDirAndNotUserData) {
      const { base: profileDirName, dir: userDataDir } = path.parse(chromiumProfile)
      return {
        profileDirName,
        userDataDir,
      }
  ...
  async setupInstance() {
    ...
    let { profileDirName, userDataDir } = await ChromiumExtensionRunner.getProfilePaths(this.params.chromiumProfile)
