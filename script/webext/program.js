...
`).help('help').alias('h', 'help').env(envPrefix).version(version).demandCommand(1, 'You must specify a command').strict().recommendCommands();
  ...
  program.command('build', 'Create an extension package from source', commands.build, {
    ...
    'upload-source-code': {
      describe: 'Path to an archive file containing human readable source code of this submission, ' + 'if the code in --source-dir has been processed to make it unreadable. ' + 'See https://extensionworkshop.com/documentation/publish/source-code-submission/ for ' + 'details. Only used with `use-submission-api`',
      type: 'string'
    }
